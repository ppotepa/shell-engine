//! Embedded 8×16 bitmap font for SDL2 overlay text rendering.
//!
//! Contains a classic VGA-compatible monospace font covering printable ASCII
//! (0x20–0x7E). Each glyph is 8 pixels wide × 16 pixels tall, stored as
//! 16 bytes where each byte represents one row (MSB = leftmost pixel).
//!
//! This avoids any dependency on SDL2_ttf or external font files.

pub const GLYPH_WIDTH: u32 = 8;
pub const GLYPH_HEIGHT: u32 = 16;

/// Returns the 16-byte bitmap for a printable ASCII character.
...
/// Classic 8×16 VGA bitmap font, printable ASCII 0x20–0x7E (95 glyphs).
/// Each glyph: 16 rows of 8 pixels packed MSB-first into one byte per row.
...
mod tests {
    ...
    fn letter_a_has_pixels() {
        assert!(glyph('A').iter().any(|&b| b != 0));
    }

    #[test]
    fn out_of_range_returns_question_mark() {
        assert_eq!(glyph('\x01'), glyph('?'));
        assert_eq!(glyph('\u{FFFF}'), glyph('?'));
    }

    #[test]
    fn all_printable_ascii_have_distinct_index() {
        for c in 0x20u8..=0x7Eu8 {
            let ch = c as char;
            let g = glyph(ch);
            assert_eq!(g.len(), 16, "glyph for '{}' should be 16 bytes", ch);
        }
    }
}
