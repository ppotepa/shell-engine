...
Planet Generator (`mods/planet-generator`) to interaktywny mod do generowania planet proceduralnych w silniku Shell Quest — terminalowym renderze 3D opartym o SDL2 pixel canvas. Mod pozwala w czasie rzeczywistym kręcić parametrami (slidery GUI) i oglądać jak planeta się zmienia.
...
- Sun-aware: dzień/noc, Rayleigh/Mie forward scatter na krawędziach
...
### Krótkoterminowe
- [ ] **Slider snapping / discrete indicators** — slidery GUI nie mają wizualnych markerów dla wartości dyskretnych (np. base mesh miał 4 opcje, atmo color ma 6). Przydałby się system ticków/snap w `engine-gui`
...
- [ ] **Atmosphere glow beyond silhouette** — per-pixel rim działa świetnie ALE jest ograniczony do pikseli na powierzchni planety. Prawdziwa atmosfera widoczna jest też poza dyskiem planety (korona). Opcje:
  - Post-process bloom/glow pass na krawędziach planety
  - Screenspace ring/halo rasteryzowany po planecie
- [ ] **Cloud layer** — planeta ma już support na `below_threshold_transparent` + `cloud_alpha_softness` w RGBA path — można dodać drugą warstwę OBJ z chmurami
...
### Znane problemy
- 3 testy w `engine` failują — `shell_quest_intro_logo_renders_non_black_cells`, `real_shell_quest_mod_manifest_and_entrypoint_load`, `real_shell_quest_scenes_all_load` — bo `mods/shell-quest` nie istnieje lokalnie (wydzielone do osobnego repo). **Nie nasze bugi.**
...
| `engine-render-3d/src/effects/atmosphere.rs` | Per-pixel rim/haze atmosphere overlay (~109 linii) |
...
| `engine-events/src/lib.rs` | `EngineEvent` / `InputEvent` (w tym nowy `MouseWheel`) |
| `engine-worldgen/src/lib.rs` | Proceduralna generacja: noise, biomy, klimat, heightmap |

## Komendy

```bash
# Uruchom planet generator
SHELL_QUEST_MOD_SOURCE=mods/planet-generator cargo run -p app

# Z debug overlay (F1=stats, ~/`=logs)
SHELL_QUEST_MOD_SOURCE=mods/planet-generator cargo run -p app -- --debug-feature
