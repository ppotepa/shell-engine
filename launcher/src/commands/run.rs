...
pub fn run(workspace_root: &Path, args: &RunArgs) -> Result<()> {
    ...
    if let Some(ref output) = args.output {
        cmd = cmd.app_arg("--output").app_arg(output);
        if output == "sdl2" {
            cmd = cmd.feature("sdl2");
        }
    }
    
    if let Some(ref mode) = args.renderer_mode {
        cmd = cmd.app_arg("--renderer-mode").app_arg(mode);
    }
    
    if args.audio { cmd = cmd.app_arg("--audio"); }
    if args.dev { cmd = cmd.app_arg("--dev"); }
    if args.no_dev { cmd = cmd.app_arg("--no-dev"); }
    if args.skip_splash { cmd = cmd.app_arg("--skip-splash"); }
    if args.logs { cmd = cmd.app_arg("--logs"); }
    if args.no_logs { cmd = cmd.app_arg("--no-logs"); }
    if args.console_log { cmd = cmd.app_arg("--console-log"); }
    ...
    if args.opt {
        ...
        if args.opt_comp { cmd = cmd.app_arg("--opt-comp"); }
        if args.no_opt_comp { cmd = cmd.app_arg("--no-opt-comp"); }
        if args.opt_present { cmd = cmd.app_arg("--opt-present"); }
        if args.opt_diff { cmd = cmd.app_arg("--opt-diff"); }
        if args.opt_skip { cmd = cmd.app_arg("--opt-skip"); }
        if args.opt_rowdiff { cmd = cmd.app_arg("--opt-rowdiff"); }
        if args.no_opt_rowdiff { cmd = cmd.app_arg("--no-opt-rowdiff"); }
        if args.opt_async { cmd = cmd.app_arg("--opt-async"); }
    }
