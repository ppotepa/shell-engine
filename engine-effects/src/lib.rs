...
pub use engine_core::effects::{Effect, EffectTargetMask, Region};
pub use metadata::{EffectMetadata, ParamControl, ParamMetadata};

use engine_core::authoring::metadata::FieldMetadata;
use engine_core::buffer::Buffer;
use engine_core::scene::{Effect as SceneEffect, EffectParams};
...
impl EffectDispatcher {
    ...
    pub fn supports_target_kind(
        &self,
        name: &str,
        target_kind: engine_core::scene::EffectTargetKind,
    ) -> bool {
        self.registry
            .get(name)
    ...
    pub fn metadata(&self, name: &str) -> &'static EffectMetadata {
        self.registry
            .get(name)
            .map(|e| e.metadata())
            .unwrap_or(&metadata::META_UNKNOWN)
    }
