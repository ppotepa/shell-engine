//! Scene/layer post-process that emulates a classic CRT display look.
...
pub static METADATA: EffectMetadata = EffectMetadata {
    ...
    sample: "- name: crt-filter\n  duration: 9000\n  params:\n    intensity: 0.6\n    sphericality: 0.12\n    transparency: 0.10\n    brightness: 0.95\n    speed: 0.45",
};

pub struct CrtFilterEffect;

impl Effect for CrtFilterEffect {
    fn apply(&self, progress: f32, params: &EffectParams, region: Region, buffer: &mut Buffer) {
        ...
        for dy in 0..region.height {
            for dx in 0..region.width {
                ...
                // Keep original glyphs to avoid duplicate-font artifacts at low render resolutions.
...
#[cfg(test)]
mod tests {
    use super::{CrtFilterEffect, METADATA};
