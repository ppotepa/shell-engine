...
- `lifecycle_controls` — GUI input fan-out (`update_gui` accepts `Vec<InputEvent>`), `sync_widget_visuals` (applies `GuiControl::visual_sync()` to scene sprites via resolver), free-look and obj-viewer mouse moves (`(f32, f32)`), terminal shell, lifecycle control routing used by engine lifecycle orchestration
