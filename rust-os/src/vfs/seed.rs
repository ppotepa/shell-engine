...
pub fn seed(vfs: &mut Vfs) {
    ...
    // --- linux-0.01 source ---
    vfs.write_file(
        "/usr/torvalds/linux-0.01/README",
        "INSTALL notes:\n\
         \n\
    ...
    vfs.write_file(
        "/usr/torvalds/linux-0.01/RELNOTES-0.01",
    ...
    // --- notes ---
    vfs.write_file(
        "/usr/torvalds/notes/starter.txt",
        "Notes to self:\n\
    ...
    vfs.write_file("/tmp/.Xauthority", "[binary Xauth data]\n", "torvalds");
    vfs.write_file("/tmp/.lock-ast", "ast:tty1:1991-09-17\n", "ast");
