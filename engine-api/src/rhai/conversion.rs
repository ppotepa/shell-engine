//! Shared Rhai/JSON/typed conversion helpers.
...
pub fn map_number(args: &RhaiMap, key: &str, fallback: rhai::FLOAT) -> rhai::FLOAT {
    args.get(key)
        .and_then(|v| {
            v.clone()
                .try_cast::<rhai::FLOAT>()
...
pub fn map_int(args: &RhaiMap, key: &str, fallback: rhai::INT) -> rhai::INT {
    args.get(key)
        .and_then(|v| {
            v.clone()
                .try_cast::<rhai::INT>()
...
pub fn map_bool(args: &RhaiMap, key: &str, fallback: bool) -> bool {
    args.get(key)
        .and_then(|v| v.clone().try_cast::<bool>())
        .unwrap_or(fallback)
}

pub fn map_string(args: &RhaiMap, key: &str) -> Option<String> {
    args.get(key).and_then(|v| v.clone().try_cast::<String>())
}

pub fn map_dynamic(args: &RhaiMap, key: &str) -> Option<RhaiDynamic> {
    args.get(key).cloned()
}

pub fn json_to_rhai_dynamic(value: &JsonValue) -> RhaiDynamic {
    match value {
        JsonValue::Null => ().into(),
...
pub fn rhai_dynamic_to_json(value: &RhaiDynamic) -> Option<JsonValue> {
    if value.is_unit() {
...
pub fn map_get_path_dynamic(map: &RhaiMap, path: &str) -> Option<RhaiDynamic> {
...
pub fn map_set_path_dynamic(map: &mut RhaiMap, path: &str, value: RhaiDynamic) -> bool {
    let segments = path
        .split('.')
...
pub fn merge_rhai_maps(base: &mut RhaiMap, patch: &RhaiMap) {
...
pub fn normalize_set_path(path: &str) -> String {
    path.trim()
        .strip_prefix("props.")
        .unwrap_or(path.trim())
        .to_string()
...
pub fn behavior_params_to_rhai_map(params: &BehaviorParams) -> RhaiMap {
    let mut out = RhaiMap::new();
...
pub fn region_to_rhai_map(region: &Region) -> RhaiMap {
    let mut out = RhaiMap::new();
