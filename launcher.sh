...
cargo run $release_flag -p app -- --mod-source $shell_mod_source${START_SCENE:+ --start-scene "$START_SCENE"}$( [[ "$SKIP_SPLASH" == "1" ]] && echo " --skip-splash" )$( [[ "$AUDIO" == "1" ]] && echo " --audio" )$( [[ "$SDL2" == "1" ]] && echo " --sdl2" )$( [[ "$CHECK_SCENES" == "1" ]] && echo " --check-scenes" )
status=\$?
