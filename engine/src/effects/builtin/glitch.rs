...
impl Effect for GlitchOutEffect {
    fn apply(&self, progress: f32, _params: &EffectParams, region: Region, buffer: &mut Buffer) {
        ...
        if progress < PHASE_DECAY_END {
                ...
                if row_n < line_drop_prob {
                    for dx in 0..region.width {
                        ...
                        if n < 0.12 * (1.0 - phase_t) {
                            buffer.set(x, y, glitch_char(n), phosphor_green(0.4 + n * 0.6), TRUE_BLACK);
                        } else {
                    ...
                    for dx in 0..region.width {
                        ...
                        if row_blink && n < 0.40 {
                            buffer.set(x, y, glitch_char(n), phosphor_green(0.65 + n * 0.35), TRUE_BLACK);
                            continue;
                        }

                        if n < dead_pixel_prob + corrupt_prob {
                            let ch = if src.symbol == ' ' { glitch_char(n) } else { src.symbol };
                            let fg = hacker_tint(src.fg, n, decay);
                            buffer.set(x, y, ch, fg, TRUE_BLACK);
                        } else {
                            let ch = if n > 0.985 { glitch_char(n) } else { src.symbol };
            ...
            let phase_t = ((progress - PHASE_DECAY_END) / (PHASE_SNOW_END - PHASE_DECAY_END)).clamp(0.0, 1.0);
...
fn snow_colour(n: f32) -> Color {
    if n < 0.33 {
        ...
        Color::Rgb { r: 120, g: 175, b: 160 }
    } else {
        Color::Rgb { r: 190, g: 215, b: 205 }
    }
}
...
fn hacker_tint(base: Color, n: f32, decay: f32) -> Color {
    ...
    if n > 0.92 {
        out = lerp_colour(out, Color::Rgb { r: 230, g: 255, b: 235 }, (n - 0.92) / 0.08);
    }
    out
}
