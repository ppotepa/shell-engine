...
#[allow(clippy::too_many_arguments)]
pub(crate) fn render_obj_to_shared_buffers(
    source: &str,
    target_w: u16,
    target_h: u16,
    mode: SceneRenderedMode,
    ...
    let Some(root) = asset_root else { return; };
    let Some(mesh) = get_or_load_obj_mesh(root, source) else { return; };
    if target_w < 2 || target_h < 2 { return; }
    let (virtual_w, virtual_h) = virtual_dimensions(mode, target_w, target_h);
    if virtual_w < 2 || virtual_h < 2 { return; }

    render_mesh_projected(
        &mesh, virtual_w, virtual_h, params, wireframe, backface_cull, fg,
        canvas, depth_buf,
    );
}
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn blit_color_canvas(
    buf: &mut Buffer,
    mode: SceneRenderedMode,
    canvas: &[Option<[u8; 3]>],
    virtual_w: u16,
