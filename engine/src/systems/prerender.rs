//! Synchronous pre-render pass: renders each eligible OBJ sprite in the scene
//! exactly once at its YAML-declared initial pose, storing the result in
//! `ObjPrerenderedFrames`. Called during scene transition when `scene.prerender == true`.
//!
//! A sprite is eligible when `rotate_y_deg_per_sec` is zero or absent — i.e. it has
//! a static rest pose.  Dynamic lights (snap_hz, flicker_hz) are fine: we capture the
//! initial frame exactly.  The caller opts specific sprites out with `prerender: false`
//! on the sprite itself.
//!
//! All eligible sprites are rendered in parallel via rayon.
...
/// Collect all prerender-eligible OBJ sprites from the layer tree.
...
fn collect_from_sprites(sprites: &[Sprite], mode: SceneRenderedMode, out: &mut Vec<PrerenderTarget>) {
    for sprite in sprites {
        match sprite {
            Sprite::Obj {
                ...
                // Opt-out: sprite explicitly disabled prerendering.
                if !prerender {
                    continue;
                }
                // Only static sprites are prerenderable.
                if rotate_y_deg_per_sec.unwrap_or(0.0).abs() > f32::EPSILON {
                    continue;
                }

                let resolved_mode = crate::render_policy::resolve_renderer_mode(mode, *force_renderer_mode);
            ...
            Sprite::Grid { children, .. } | Sprite::Flex { children, .. } | Sprite::Panel { children, .. } => {
                collect_from_sprites(children, mode, out);
            }
            _ => {}
        }
    }
...
/// Synchronously pre-render all eligible OBJ sprites in the scene.
///
/// Blocks during scene transition until all sprites are rendered (rayon parallel).
/// Registers `ObjPrerenderedFrames` and `ObjPrerenderStatus::Ready` into the world on success.
pub fn prerender_scene_sprites(
    ...
    engine_core::logging::info(
        "engine.prerender",
        format!("scene={scene_id}: prerendering {} OBJ sprites (parallel)", targets.len()),
    );

    // Render all sprites in parallel — each is fully independent.
    let results: Vec<(String, PrerenderedFrame)> = targets
        ...
        .filter_map(|target| {
            ...
            let (target_w, target_h) = obj_sprite_dimensions(target.width, target.height, target.size);
